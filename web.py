...
def main():
	...
	def create_user(username: str, password: str):
		...
		try:
			conn.execute("INSERT INTO users (username, password_hash, salt) VALUES (?, ?, ?)", (username, h, s))
			conn.commit()
			return True
	...
	def authenticate_user(username: str, password: str):
		cur = conn.execute("SELECT id, password_hash, salt FROM users WHERE username=?", (username,))
		row = cur.fetchone()
		if not row:
			return None
